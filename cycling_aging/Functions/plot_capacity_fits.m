...
for cellNum = unique(cellNums,'stable')'
    ...
    if flag_individualPlots
        ...
        title(ax1, sprintf("%0.0f \\circC, %0.0f%% SOC\n %0.0f%% DOD, %.2f C", cell_data.TdegC(1), cell_data.soc(1)*100, cell_data.dod(1)*100, cell_data.Cavg(1)));
...
if isfield(plotOpt, 'layout')
    ...
    % default single axis, do nothing 默认不绘制置信区间
    flag_individualPlots = 0;
end
...
sumR_CI = prctile(sumR_boot, confidenceInterval); % 计算百分位数
